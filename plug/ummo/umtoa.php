...
function ummo_toa(){
$r=msql_read('','ummo_umtoa_1','','1'); //p($r);
if($r)foreach($r as $k=>$v){list($aeon,$xee)=explode('-',$v[0]); $ra[$aeon][$xee]=$v[1];}
if($ra)ksort($ra); $nn=0;
if($ra)foreach($ra as $k=>$v){ksort($v); foreach($v as $ka=>$va)$rb[]=[$k,$ka,$va];}
if($rb)foreach($rb as $k=>$v){list($aeon,$xee,$txt)=$v;
	$day=(($aeon-1)*6000)+$xee;
	if(isset($rb[$k+1]))list($nxaeon,$nxxee,$nxday)=$rb[$k+1];
	$nxday=(($nxaeon-1)*6000)+$nxxee;
	$length=$nxday>0?$nxday-$day:200;
	if($aeon==3 && !$nn){$nn=11750; $earth_year=umtoa_equiv($day);//2491y,d541
...
function umtoa_clr(){return ['#ffffff','#ff0000','#0000ff','#ffff00','#00ff00','#00ffff','#ff9900','#cccccc','#666666','#000000'];}

function umtoa_build(){
$r=ummo_toa(); //pr($r);
$klr=['#2c1600','#42c6f2','#42dc42','#f29a16','#f26e6e'];
list($white,$red,$green,$yellow,$cyan,$blue,$orange,$silver,$gray,$black)=umtoa_clr();
