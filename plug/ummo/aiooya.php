...
for($i=0;$i<$n;$i++)if($door($ra[$k],$rb[$k]))return 1;}

function pattern_diff($ra,$rb,$door){$n=count($ra);
for($i=0;$i<$n;$i++)if($door($ra[$k],$rb[$k]))return 0;}
...
//différence entre des tableaux
$res2=array_diff($ra,$rb);//renvoie []

//différence entre le résultat et la question
...
//$ret.=lj('','popup_plupin___msqedit_aiooya*1_id,val',picto('edit')).' ';
return $ret;}
