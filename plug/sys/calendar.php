...
function calendar_build($time,$to){$rid='cld'.randid();
...
for($i=1;$i<=$nbdy;$i++){$mk=mktime(0,0,0,$month,$i,$gd['year']); $day=date('d',$mk);
	...
	//$bt=lj('lina'.$c,$to.'_plug__4x_calendar_calendar*v_'.$date,$day)."\n";
...
function calendar_v($p,$o){return $p;}

function calendar_j($p,$o,$res=''){
list($p,$o)=ajxp($res,$p,$o);
return calendar_build($p,$o);}
