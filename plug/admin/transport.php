...
static function last($db){
return sql_b('select id from '.self::pub($db).' order by id DESC limit 1','v');}
...
static function pub($b){$r=['art','txt','trk','data','meta','meta_art','search','search_art','poll','twit','user','web','yandex','ips','live','stat','cat','hub','mbr','img','umvoc','umvoc_arts','umtwits'];
if(in_array($b,$r))return qd($b); else return $b;}

static function db_r(){
...
static function srv(){$srv=$_SERVER['HTTP_HOST'];
$sdr=isset($_SESSION['local'])?'':'/home';//$_SERVER['DOCUMENT_ROOT']
$d=read_file('params/_connectx.php');
$d=between($d,'mysqli_connect(',')');
$d=str_replace("'",'',$d); $r=explode(',',$d); //pr($r);
require('params/_connectx.php');
if(auth(6))return ['root',$db,$r[2],$sdr.'/'.$db];}
...
[$usr,$db,$ps,$dr]=self::srv(); $res=''; $root=__DIR__; //$root=''.$dr;
...
if($o=='z' && auth(7)){sqldrop(self::ts_db($p));//$b=self::pub($p); qr('drop table '.$b);//reinit
	$r=install::db(); $d=$r[$p]; if($d)qr($d); $res=$p.':z';}
elseif($o=='zz' && auth(7)){$r=self::tables();//reinstal tables
	foreach($r as $k=>$v)sqldrop(self::ts_db($v));//qr('drop table '.self::pub($v));
	return install::home(ses('qd'));}
elseif($o=='json'){//dj
	...
	if($d && !$er){$ra=self::db_r(); $b=$ra[$p]; $bb=sqlbcp($b); trunc($b); sqlsav2($b,$r,1,0); $res=$b.':renoved';}}
else{//partial and complete dumps, not gziped
	//$ra=self::db_r(); $b=$ra[$p]; $bb=sqlbcp($b); trunc($b);
...
$r=self::tables(); $ret='';
foreach($r as $k=>$v)$ret.=self::call($v,$p);
return $ret;}
