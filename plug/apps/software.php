...
static function rapport($r){return lj('txtalert','updb_plug___pubdate',count($r[0]).' files updated, '.count($r[1]).' files created, '.count($r[2]).' files deleted');}
...
if(auth(7))$bt.=lj('txtx','upd_plug___dev2prod',pictxt('upload','push'));
if(auth(7))$bt.=lj('txtx','upd_plug___pubdate',pictxt('export','publish site'));
