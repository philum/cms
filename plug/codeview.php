...
function save_func_sqi($day,$resav,$func,$page){
msquery('INSERT INTO _sys VALUES ("","'.$func.'","'.$page.'","'.$day.'","'.$resav.'")');}//msquery
function save_func_squ($day,$resav,$id){
msquery('UPDATE _sys SET maj="'.$day.'", func="'.$resav.'" WHERE id='.$id);}

function save_funcs($f,$page){//echo $f.' ';
...
if(is_file($f)){
	...
	foreach($reb as $func){
	if($func){
		list($id,$maj)=$_SESSION['sys'][$func];
		//list($id,$maj)=sql_b('select name,page from _sys where name="'.$func.'"','vv');
		$res=find_end($rea,'function '.$func.'(','{','}'); 
		//$res=substr($res,strpos($res,'{')+1,strrpos($res,'}'-1)); //eco($res,1);
		$resav=addslashes($res);
		if(!$id && $maj!=$day)save_func_sqi($day,$resav,$func,$page);
		elseif($maj!=$day)save_func_squ($day,$resav,$id);
		$ret.='//'.$func."\n";
		$ret.=$res."\n\n";}}
...
$r=array('admin','adminx','ajax','ajxf','art','boot','bubs','finder','lib','meta','mod','msql','pop','sav','spe','styl','tri');//,'ajx.js','utils'
foreach($r as $v)save_funcs('progb/'.$v.'.php',$v);}//define_sys();

function savefunc_plug(){//save_all;
$r=explore('plug','',1); //p($r);
foreach($r as $v)if($v!='userdl.tar.gz')save_funcs('plug/'.$v,$v);}

function define_sys(){//id,name,page,maj,func
$_SESSION['sys']=sql_b('select func,id,maj,page from _sys','kvv');}
...
#pub
/*function xhtml_highlight($str){$str=highlight_string($str,true);
...
define_sys(); //if(!$_SESSION['sys'])//pr($_SESSION['sys']);
...
if($dr=='save'){$_GET['sav']=1; $dr=$_SESSION['dr']; $dr=='plug'?savefunc_plug():savefunc();}
