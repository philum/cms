...
foreach($mnu as $k=>$v){$pb=ajx($o.$v,0);
	$tts[]=lj('','msqt_plug___msqread_msqt*read_'.ajx($nod,0).'_'.$o.$k,$v);}
foreach($r as $k=>$v){
...
function plug_msqread($nod,$tmp){
//require_once('plug/msqads.php');
$ret=lj('txtbox','msqt_plug___msqread_plug*msqread_'.ajx($nod).'_'.ajx($tmp),picto('reload')).br();
