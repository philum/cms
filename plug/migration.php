...
$servr=define_s('servr','philum.org');//target
$target_hub=define_s('target_hub','');//target
...
if($r)foreach($r as $k=>$v){echo $v.';';}}
//php
if($_GET['php']){
$r=scrut_dir_only_tri('../'.$_GET['php']);
if($r)foreach($r as $k=>$v){echo str_replace(array('<'.'?php','?'.'>'),'',$v).';';}}
//dir
...
//data
//http://philum.net/params/newsnet_backup_pub_art_from_56257.txt
...
foreach($dirs as $k=>$v){echo lka('?target_hub='.$h.'&block='.$v,$v).' ';}
echo br();
...
if($_GET['block']=='ban'){echo lka('?block=ban&import=ban',$txt).br();;}
if($_GET['block']=='datas'){$txt='enter like "pub_art_from_56257" ';
	echo lka('?block='.$v.'&data=',$txt).br();
	if($_GET['data'])dl_data($h);}
}

echo 'follow_the_rabbit :: fill asked variables in URL'.br();
if($_SESSION['target_hub'] && $_SESSION['auth']>=6)import_hub();
echo br().br();
echo lka($root.'/?servr=','enter target_server').br();
echo 'target_server: '.$_SESSION['servr'].br();
echo lka($root.'/?target_hub=','target_hub').br();
echo 'target_hub: '.$_SESSION['target_hub'].br();
echo 'local_hub: '.$_SESSION['qb'].br();
