...
static function import($defs,$db){//from msql
...
$sq='select COLUMN_NAME,DATA_TYPE,CHARACTER_MAXIMUM_LENGTH from INFORMATION_SCHEMA.COLUMNS where table_name="'.$b.'"';
$rq=qr($sq);
while($r=mysqli_fetch_assoc($rq)){$type=$r['DATA_TYPE']; $sz=$r['CHARACTER_MAXIMUM_LENGTH']; $nm=$r['COLUMN_NAME'];
	...
	elseif($type=='tinytext')$type='tiny';
...
foreach($r as $k=>$v)
	if($v=='ai')$ret.='`id` int(11) NOT NULL auto_increment,';
	elseif($v=='int')$ret.='`'.$k.'` int(11),'."\n";
	elseif($v=='sint')$ret.='`'.$k.'` int(7),'."\n";
	elseif($v=='2int')$ret.='`'.$k.'` int(2),'."\n";
	...
	elseif($v=='2var')$ret.='`'.$k.'` varchar(2) NOT NULL default "",';
	elseif($v=='tiny')$ret.='`'.$k.'` tinytext NOT NULL default "",';
	elseif($v=='text' or $v=='med')$ret.='`'.$k.'` mediumtext NOT NULL default "",';
...
if($up && $ra && $ra!=$r){$sql=self::trigger($b,$r); //echo $sql;
	if($sql)qr('create table if not exists `'.qd($b).'` (
	  '.self::create_cols($r).'
	  PRIMARY KEY (`id`)
	) ENGINE=MyISAM collate latin1_general_ci;',1);}
