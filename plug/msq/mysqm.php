...
function mq_edit($p,$o,$res=''){[$p,$o]=ajxp($res,$p,$o);
$r=sql('select '.$p.' from '.$o.';','');
//if($r)foreach($r as $v)$ret.=lj('','popup_plug___mysqm_mq*edit_'.$v[0],$v[0]);
return $ret;}

function mq_cols($p,$o,$res=''){[$p,$o]=ajxp($res,$p,$o); //echo $p."-".$o;
...
$ret[]=array(lj('txtx','popup_plug___mysqm_mq*edit_'.$v[0],$v[0]),$v[1],$v[2]);
return tabler($ret);}

function mq_tables($p,$o,$res=''){[$p,$o]=ajxp($res,$p,$o);
$r=lstrc(rcptb($p)); 
