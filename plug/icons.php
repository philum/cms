...
function icons_pick(){$r=icons_clrs();
for($i=0;$i<10;$i++){$ret.=ljb('bk'.$i,'icons_pick('.$i.')','',' ');}
return divc('pixels',$ret);}

function icons_css(){$r=icons_clrs();
...
function icons_sav(id,nw){var ret=icons_rcp(id);
	...
	SaveJ("socket_plug__4_icons_icons*sav_"+nam+"_"+ret); sav_btn("cbk"+id);
...
for($y=0;$y<$h;$y++){for($x=0;$x<$w;$x++){
	list($rg,$gb,$br)=$clr[$r[$i]]; $clri='clr'.$r[$i];
	if($r[$i])imagesetpixel($im,($x),($y),$$clri);
...
$d=msql_read('system','program_icons',$k); if($d)$r=str_split($d);
icons_graphics($r,16,16,$nm); return pictosys($k);}

//sav
function icons_sav($id,$d){$dfb['_menus_']=array('data');
$r=msql_modif('system','program_icons',array($d),$dfb,'one',$id);}

function icons_del($d){$dfb['_menus_']=array('data');
...
//edit
function ljd($d,$i){
//return '<a onclick="icons_toggle(this,'.$i.')" ondragover="icons_toggle(this,'.$i.')" class="bk'.$d.'"> </a>';
...
//read
function pictosys($d){$nm=$nm.'?time='.randid();
	return imgico('../imgb/icons/system/philum/16/'.$d.'.png'.$nm,$nm,4,'../');}

function icons_read(){$id=randid();
$r=msql_read('system','program_icons',''); if($r)unset($r['_menus_']); ksort($r);
if($r)foreach($r as $k=>$v){$im=pictosys($k).' ';
	//icons_graphics($v,16,16,'../imgb/icons/system/philum/16/'.$k.'.png');
	$ret[]=lj('popbt','popup_plup___icons_icons*edit_'.$k,$im.$k).br();}
