...
function taxonav_read($h,$i){req('spe');
$r=collect_hierarchie_d("reverse");
$r=tri_hierarchic($r,$h); //p($r);
if(substr($i,0,1)=='x'){$i=substr($i,1);} else{$i='x'.$i; $o='x';}
if($o)$p='&#9660;'; else $p='&#9658;';//down-right //
$nb=btn('txtsmall2','('.count($r).')');
$ret=balc("li",'" style="list-style-type:none;','&#9500;&#150;'.good_gb($h,$i,$p).' '.$nb);
if($r)return $ret.balc("ul","taxonomy",make_menus_rb($r,$h,$o));}
...
$ret.=balc("ul","taxonomy",make_menus_rb($r,'',1));
