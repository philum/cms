...
return lj('" title="'.val($r,0).' ('.$r11.')','popup_plup___spitable1_spi*infos_'.$r11,divs($sty,$r11.br().$r11));}
...
foreach($r as $k=>$v){$c=$v<$d?'active':'';
	$ret.=lj($c,'spi_plug__2_spitable1_spi*table_'.$v,btn('',$v)).' ';}
return $ret;}

function spi_menu($d){$d=$d?$d:118;
if($d>1)$ret=lj('txtbox','spi_plug__2_spitable1_spi*table_'.($d-1),picto('before')).' ';
$ret.=input1('spix',$d,'','','1').' ';
$ret.=lj('txtbox','spi_plug__2_spitable1_spi*table_'.($d+1),picto('after')).' ';
$ret.=lj('txtbox','spi_plug__2_spitable1_spi*table___spix','set');
