...
$ret='[spl_app__2_spiline2_call_'.$n.'_'.$mode.'§['.($x+4).','.($y+15).',12§'.$n.':text]:lj]';
if($ratio==2)$ret.='['.($x+4).','.($y+35).',12§'.$valence.':text]';
$ret.='[popup_plup___spitable_spi*infos_'.$n.'§['.($x+$w/($ratio==2?2:2)).','.($y+(15*$ratio)).','.(12*$ratio).'§'.$v.':text]:lj]';
//echo $x.'-'.$y.'-'.$w.'-'.$h.br();
return [$x,$y,$w,$h,$ret];}
...
$j=$rid.'_app__2_spiline2_call___inpsl2';
$ret=inputj('inpsl2',$p,$j).lj('',$j,picto('ok')).' ';
return $ret;}

static function nav($p,$o,$rid){
$ret=self::menu($p,$o,$rid); $bt='';
if($p>0)$ret.=lj('',$rid.'_app__2_spiline2_call_'.($p-1).'_'.$o,picto('previous')).' ';
if($p<self::$max)$ret.=lj('',$rid.'_app__2_spiline2_call_'.($p+1).'_'.$o,picto('next')).' ';
$ra=[1=>'left','valences/left','right','valences/right'];//,'angular'
foreach($ra as $k=>$v)$bt.=lj(active($k,$o),$rid.'_app__2_spiline2_call_'.$p.'_'.$k,$v);
$ret.=btn('nbp',$bt);
...
function plug_spiline2($p,$o){
new spiline2(118);
$rid='spl'; $p=$p?$p:spiline2::$max; //reqp('spt');
Head::add('csscode',spiline2::css());
//Head::add('jscode',spiline2::js($rid.'_app__2_spiline2_call',$p,$o));
$ret=spiline2::build($p,$o);
return divd($rid,$ret);}
