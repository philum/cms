...
$ret.=lj('txtbox','spigrow_app__2_spigrow_j___spigrowx','ok').' ';
if($d>1)$ret.=lj('txtbox','spigrow_app__2_spigrow_j_'.($d-1),picto('before')).' ';
$ret.=lj('txtbox','spigrow_app__2_spigrow_j_'.($d+1),picto('after')).' ';
$ret.=hlpbt('spitable');
...
$ret=lj('','spigrow_app__2_spigrow_j_'.$n,$n).' :: ';
//$ret.=lj('" title="'.$r[0],'popup_plup___spigrow_spigrow*infos_'.$n,$r[1]);
$ret.=lj('" title="'.$r[0],'popup_app___spigrow_infos_'.$n,$r[1]);
return divs($sty,$ret);}
...
function plug_spigrow($d){if(!$d)$d=118;
$ret=spigrow::table($d);
$pub=divc('small','SpiTable-Davy@2016');
return divd('spigrow',$ret).br().spigrow::fams().$pub;}
