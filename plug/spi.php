...
function spi_classes(){return array(''=>'ccc','Nonmetals'=>"a0ffa0",'Nobles Gasses'=>"c0ffff",'Alkali Metals'=>"ff6666",'Alkali Earth Metals'=>"ffdead",'Metalloids'=>"cccc99",'Halogens'=>"ffff99",'Metals'=>"cccccc",'Transactinides'=>"ffc0c0",'Lanthanides'=>"ffbfff",'Actinides'=>"ff99cc",'undefined'=>"ffffff");}

function spi_classes_fr(){return array(''=>'ccc','Non-métaux'=>"a0ffa0",'Gaz nobles'=>"c0ffff",'Métaux alcalins'=>"ff6666",'Métaux alcalino-terreux'=>"ffdead",'Métalloïdes'=>"cccc99",'Halogènes'=>"ffff99",'Métaux pauvres'=>"cccccc",'Métaux de transition'=>"ffc0c0",'Lanthanides'=>"ffbfff",'Actinides'=>"ff99cc",'undefined'=>"ffffff");}
...
function spi_fams(){$r=spi_classes_fr();
foreach($r as $k=>$v){
...
foreach($r as $k=>$v){$c=$v<$d?'active':'';
	$ret.=lj($c,'spi_plug__2_spi_spi*table_'.$v,btn($sty,$v)).' ';}
return $ret;}

function spi_menu($d){$d=$d?$d:118;
if($d>1)$ret=lj('txtbox','spi_plug__2_spi_spi*table_'.($d-1),picto('left')).' ';
$ret.=input(1,'spix',$d,'','','1').' ';
$ret.=lj('txtbox','spi_plug__2_spi_spi*table_'.($d+1),picto('right')).' ';
$ret.=lj('txtbox','spi_plug__2_spi_spi*table___spix','set');
