...
function pet_hlp($d){return msql::val('lang','helps_petition',$d);}
...
function pet_form($id){$r=['name','mail','web','city','country']; $ret=''; $ids='';
...
function pet_count_valid($r){$i=0; foreach($r as $k=>$v){if($v[6]==1)$i++;} return $i;}
...
function pet_j($d){[$nba,$id,$page]=explode('|',$d); $_GET["read"]=$id;
$r=msql_read('',nod('petition_'.$id));
...
$ret=lj('txtbox','popup_plug___petition_pet*form_'.$id,pet_hlp('sign')).$msq.br();
if(get('confirm')){$ret.=pet_confirm($r,$nod,$id); $r=msql_read('users',$nod);}
