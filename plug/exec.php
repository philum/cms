...
function exc_fast(){$ref=array('{}','[]','if()','foreach($r as $k=>$v)','$ret=;','strpos($d,\'x\')!==false','return $ret;','.br()',"\r");
...
$d=str_replace('mysql','',$d);
if(auth(6))return eval($d);}
//exc(ajxg($res));
...
if($_SESSION["auth"]<6)$btn.=btn('txtalert','need auth>=7');
	...
	$btn.=lj('txtx',"exec","x").br();}
$ret.=txarea('txtarea" class="console" onkeyup="'.$sj.'" onclick="'.$sj,$p,61,18);
return divc('row',divc('cell',$btn.$ret).div(atd($rid).atc('cell'),''));}
