...
qr('delete from '.$_SESSION['qdd'].' where val="lang"'); reflush('qdd',1);}
...
//foreach($r as $k=>$v)insert('qdd','(NULL,"'.$v[1].'","fav","'.$v[2].'")');
}

function patch_fav(){
$r=msql_read('',ses('qb').'_fav','','1'); p($r);
foreach($r as $k=>$v){$rb=explode(' ',$v);
	//foreach($rb as $vb)insert('qdd','(NULL,"'.$vb.'","fav","'.$k.'")');
	}
}
...
foreach($r as $k=>$v){
	...
	sqldel('qda',$id); sqldel('qdm',$id);}
$r=sql('id,name','qda','kv','frm="user"'); p($r);
foreach($r as $k=>$v){sqldel('qda',$k); sqldel('qdm',$k);}}
...
while($data=mysqli_fetch_array($rq)){$prm=''; $dthm=$data['thm'];
...
$rq=res("id,thm",$_SESSION['qda'].' WHERE nod="'.$_SESSION['qb'].'"');
while($data=mysqli_fetch_array($rq)){$thm='';
