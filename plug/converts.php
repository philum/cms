...
function codage($txt,$d){
if($_POST['Submit']==$d.'_encode')$enc=1;
...
elseif($d=='php')$ret=$enc?clean_code($txt):$ret;
return stripslashes($ret);}

function plug_converts($p){//setlocalvar("convert","utf8");
$menu=array("utf8","htmlentities","url","unescape","base64","ascii","binary","bin","php");
foreach($menu as $val)$ret.=lkc("txtx",'/plugin/converts&p='.$val,$val).' ';
$ret.='<form id="form1" name="form1" method="post" action="">
  <input type="submit" name="Submit" value="'.$p.'_encode" />
  <input type="submit" name="Submit" value="'.$p.'_decode" /><br>
  <textarea name="txt" cols="34" rows="22" class="console" style="float:left;">'.stripslashes($_POST['txt']).'</textarea>';
$res=htmlentities(codage($_POST['txt'],$p));
//$ret.=divc('tab justy',$res);
$ret.='<textarea name="txtb" cols="34" rows="22" class="console">'.$res.'</textarea></form>';
return $ret;}
?>