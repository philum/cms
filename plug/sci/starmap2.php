...
$x=60; $j='stm2_app__2_starmap2_call_'.$p1.'_'.$p2.'-';
...
$x=60; $j='stm2_app__2_starmap2_call_'.$p1.'_';
...
list($white,$black,$red,$green,$blue,$yellow,$cyan,$orange,$silver,$gray)=starlib::$clr;//spe
...
foreach($r as $k=>$v){
	...
	//svg::lj($xb,$yb,12,$white,'popup_app___star_call_'.$hip.'_1','HIP '.$hip);
	//svg::bubj($xb,$yb,12,$white,'app___star_info_'.$v['hip'].'_hip',$nm);
	$clr=$ds>$p2?'gray':'white';
	svg::$ret[]='['.$clr.':attr]['.$tx.',app___star_info_'.$hip.'_hip§['.$xb.','.$yb.',12§'.$nm.':text]:bubj2]';}}
...
static function call($p1,$p2,$res=''){
list($p1,$p2)=ajxp($res,$p1,$p2);
list($p2,$ob)=expl('-',$p2);
//if($p3)self::$w=$p3;
$ret=self::build($p1,$p2,$ob);
return divb($ret,'','stm2');}

static function menu($p1,$p2,$rid){
$j=$rid.'_starmap2,call__xr_____p1|p2|p3';
$ret=inputj('p1',$p1?$p1:self::$default,$j,atz(36));
$ret.=lj('',$j,picto('ok')).hlpbt('starmap').' ';
$ret.=inputj('p2',$p2?$p2:50,$j,atz(4)).label('p2','(limit horizon)').' ';
//$ret.=inputj('p3',1400,$j,atz(4)).label('p3','size').' ';
$ret.=lj('txtx',$rid.'_app__2_starmap2_call_'.self::$default,'default').' ';
$ret.=lj('txtx',$rid.'_app__2_starmap2_call_knownstars_50','known').' ';
$ret.=lj('txtx',$rid.'_app__2_starmap2_call_allstars_50','all').' ';
//$ret.=lj('txtx',$rid.'_app__2_starmap2_call_HD20766,HD61606,HD32923,HD217877,HD23065','serpo').' ';
return $ret;}
...
function plug_starmap($p1,$p2){
return starmap2::home($p1,$p2);}
