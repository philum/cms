...
$j='strmp4_app__2_starmap4_call_'.$p;
...
foreach($r as $k=>$v){if(!is_numeric($v))$n=0.5; else $n=($v-$min)*$ca; 
...
list($r0,$r1,$r2,$r3)=starlib::zpt(); $r0b=$r0; $r3b=$r3; $cx=12; $op=0.6; $bdr='#999999'; //$bdr='';
...
$n=100; for($i=0;$i<32;$i++)$r[]=self::declinaison($a,$n,$i,3);
foreach($r as $k=>$v)$rb[]=round(($k)*$mw,2).'/'.round($hm-$v*$mh,2);
...
if($p=='knownstars' or $p=='allstars'){
	if($p=='allstars'){$rb=msql::read('','ummo_exo_stars','',1); $ra=array_merge($ra,$rb);}
	$pb=implode(',',array_keys_r($ra,8));}
$sq=star::sq($pb);
$r=star::build($sq,1); //pr($r);
$rb=starlib::prep($r,$ra,$p); //pr($rb);
$rb=starlib::positions($rb);
$ret=self::draw($rb,$p,$o);
svg::save($ret,'starmap4');
return $ret;}
...
$ret.=lj('txtx',$rid.'_app__2_starmap4_call_knownstars','known').' ';
$ret.=lj('txtx',$rid.'_app__2_starmap4_call_allstars','all').' ';
