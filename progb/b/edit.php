...
//menus
static function icon($v){$rp=sesmk2('pop','msqmimes','',0);//pictos
...
foreach($r as $k=>$v)
	if($v=='ascii')$ret.=lj('','popup_mc,navs___ascii4_'.$id,$v); 
	else $ret.=togbub('mc,navs',$v.'_'.$id,$v);//$v=='del'?1:
...
//f
static function artform($link,$id){$_SESSION['cur_div']='content'; $ip=hostname();
...
if(!$id)$r['urlsrc']=input1('urlsrc',$link,16,'','url','255',['onclick'=>atj('SaveI','urlsrc'),'onContextMenu'=>'SaveIt()']).btd('urledt','');
if($USE && !$id){
	$r['trkname']=hidden('trkname',$USE).hidden('trkmail','');
	$r['slcat']=select_j('frm','cat',$frm,'3',$frm,'');}
elseif(!$USE){$gn='" onkeyup="log_goodname(\'trkname\');';
	$r['trkname']=input1('trkname'.$gn,$us,8,'',nms(38),'50');
	$r['trkmail']=input1('trkmail',$ml,12,'email','mail','50');}//mail
...
if($id){$msg=sql('msg','qdm','v','id='.$id); $alrt=self::correct($msg);}
...
if($id){$sav=lj('popbt','art'.$id.'_sav,editart_txtarea_id4_'.$id,picto('save'));
	$sav.=lj('popsav','txarea,art'.$id.'_sav,editart_txtarea_id4_'.$id.'_1',nms(57));}
else $sav=lj('popsav','socket_sav,createart_txtarea,'.$ids.'_'.(rstr(57)?7:9),nms(57));
