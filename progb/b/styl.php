...
//static function actions($p,$o,$b,$res){
//static function actions($rg,$prm){[$p,$o,$b]=$rg;
static function actions($p,$o,$prm){$res=$prm[0]??'';
$bd='design'; $bs='system';
self::clrpckr_js(); $qb=ses('qb'); $lh=sesmk2('adm','csslang');
...
case('save'):msql::save($bd,$nod,$defs);
	self::build_css($ftmp,$defs); self::build_css($fcss,$defs); break;
...
case('exit_design'):sesz('desgn'); $_SESSION['clrset']='';
	boot::define_mods(); boot::define_condition(); boot::define_clr(); ses::$popadm['design']='';
...
return lj($c.$tt,$tg.'_styl,actions_'.$id.'_'.$x.'_'.ajx($p).'_'.ajx($o),$t);}
//return bj($c.$tt,$tg.'|styl,actions|'.$x.'|'.$p.','.$o.'|'.$id,$t);}
//$ret.=self::cssactbt('action',$bt,'self',$o,'title',$res);

static function edit_css(){
self::clrpckr_js(); $qb=ses('qb'); $lh=sesmk2('adm','csslang'); $defs='';
$defsb=['div','class','element','color','bkg','border','free'];
...
$ret.=br();
$ret.=lj('txtbox','popup_styl,chargesets','design:'.$desgn.'/clrset:'.$clrset).' ';
...
$ret.=lj('txtbox',$id.'_styl,dsnamedt_inp'.$i.'__'.$nd,'ok');}
else $ret=lj('txtblc',$id.'_styl,dsnamedt_zero__'.$nd,$p);
...
if($k)$ret=toggle($csa.$css.$sty.'190px;','css'.$k.'_styl,editcss_'.$k,$t);
else $ret=btn($csa.$sty.'190px;',$t);
for($i=3;$i<6;$i++)$ret.=btn($csa.$sty.'50px',self::petit_clr($p[$i]??'-',$clr));
...
function SaveClr(nb){var clrs='/';
	...
	clrs+=va+'/';}
	//new AJAX(jurl()+'saveclr_'+clrs,'clrreponse');
	//ajaxcall('clrreponse','styl:saveclr_'+clrs,'','');
	bjcall('clrreponse|styl,saveclr||'+clrs);
...
$ret.=lj('popsav','fnt'.$k.'_styl,ffeditcall_fntcat'.$k.'__'.$k.'_cat','ok').' ';
return $ret;}

static function preview_ff_p($k,$v){$go='fnt'.$k.'_styl,ffeditprw___'.$k; $sz='';
...
//rech
$srch=input1('srchfnt',val($pp,'rch','search'),8);
$srch.=' '.bj('popbt','popup|styl,ffcall|x|1,'.$b.','.$c.','.$o.'|srchfnt','ok').' ';
...
for($i=1;$i<=$nb;$i++){
	if($i==1 or $i==$nb or ($i>$min && $i<$max))
		$rtp.=lj(($i==$p?'active':''),'popup_fontface__x_'.$i.'_'.$bb.'_'.$c.'_'.$o,$i).' ';
...
$ret.=input1('ffwr',$_SESSION['ffstr']?$_SESSION['ffstr']:'AaBbCcDdEe0123יא','44');
...
$ret.=lj('popbt','css'.$k.'_styl,savcss_'.$ids.'__'.$k.'_3',nms(66)).br().br();
return $ret;}
...
for($i=0;$i<=$n;$i++){$ri=val($r,$i); $t=self::mnu_line_t($ri,$i);
	$ret.=lj('','bt'.$p.'_styl,setclr___'.$ri.'_'.$i.'_'.$p,$t);}
return $ret;}

static function setclr($clr,$n,$p){
$h=hidden($p,$n); $t=self::mnu_line_t($clr,$n);
return togbub('styl,selectclr',$p.'_'.$n,$t).$h;}
...
$ret.=lj('popbt','css'.$k.'_styl,savcss_'.implode(',',$rt).'__'.$k.'_1',nms(66)).br();
return $ret;}
...
$ret.=label('sdx'.$k,'position:','txtx').select(atd('sdx'.$k),$rb,'kv',$k);
...
$ret.=lj('','popup_styl,savcss_cssarea'.$d.'__'.$d.'_4',picto('export')).' ';
$ret.=lj('popsav','css'.$d.'_styl,savcss_cssarea'.$d.'__'.$d,nms(66)).' ';
return divc('',$ret);}
...
static function build_css($fcss,$defs,$clr=''){unset($defs['_menus_']);
...
if(get('cmpq'))self::write_css_c($fcss,$ret); else self::write_css($fcss,$ret);
return $ret;}
}
