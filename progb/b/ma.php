...
static function lastid($bs){$wh=$bs=='qda'?'where id>="'.self::lastartid().'" ':'';
return sqb('id',$bs,'v',$wh.'order by id desc limit 1');}
static function lastartid(){if(is_array($_SESSION['rqt']))return key($_SESSION['rqt']);}
static function lastartday(){if(isset($_SESSION['rqt'])){$id=key($_SESSION['rqt']); return self::rqt($id,'suj');}}
static function lastart($d=''){$ld=$d?$d:self::lastartday(); return sql('id','qda','v','nod="'.ses('qb').'" and frm!="_system" and day>="'.$ld.'" and re>="1" order by id desc limit 1');}
static function oldestart(){
return sql('day','qda','v','nod="'.ses('qb').'" and re>="1" and frm!="_system" order by day asc limit 1');}
static function is_public($id){return sql('id','qda','v',$id.' and re>="1" and substring(frm,1,1)!="_"');}
...
return sql('id','qda','v','suj="'.$id.'" and nod="'.ses('qb').'" order by id asc limit 1');}//and re>="1"
static function ib_of_id($id){$ib=self::rqt($id,'ib'); if($ib && $ib!='/')return $ib;
elseif(!$ib && $id)return sql('ib','qda','v',$id);}
static function id_of_ib($ib){return sql('id','qda','k','ib="'.$ib.'" limit 1');}
static function suj_of_id($id){$suj=self::rqt($id,'suj'); if($suj)return $suj;
$suj=sql('suj','qda','v','id="'.$id.'"'); if(is_string($suj))return $suj;}
static function data_val($v,$id,$val,$m=''){$sq=$id?'ib="'.$id.'" and ':'';
return sql($v,'qdd',$m?$m:'v',$sq.'val="'.$val.'"');}

static function find_cat($nbj){$w='nod="'.ses('qb').'"';
...
if(!$id){$id=sql('id','qda','v','nod="'.ses('qb').'" and re>="1" and suj like "%'.$d.'%" limit 1');
	if($id){$suj=self::suj_of_id($id); $thm=hardurl($suj); update('qda','thm',$thm,'id',$id);}}
return $id;}
...
static function tags_list_nb($cat,$nbday='30'){$wh=$cat?'and cat="'.$cat.'" ':'';
