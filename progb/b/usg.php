...
static function delconn($g1){$d=sql('msg','qdm','v','id='.$g1); 
$d=html_entity_decode($d,true,$_SESSION['enc']);
$d=codeline::parse($d,'','delconn'); return clean_lines($d);}
...
static function poptxt($g1){return $ret=div(atc('twit').ats('display:block; min-width:440px;'),sesr('temp',$g1));}
static function popfile($g1){return nl2br(cleanmail(read_file($g1)));}
...
if($r)return divd('iv'.$iv,self::playvideo($iv,$_SESSION['cur_div'],0));}}
...
//dropmenu
static function dropmenu_jb($r,$id,$rid,$opt='',$n=''){//catslct//dropmenuj_prep
...
if($r){foreach($r as $k=>$v){$j='';
	...
	$ret.=ljb($c,$j,'',$k);}}
$ret=divc('list',$ret);
if($opt==2)$ret.=self::dropmenuform($id,$rid,'','adcat',$opt);
return $ret;}
...
static function dropmenuform($id,$rid,$v,$pre,$o){//mc::assistant($id,$j,$jv,$va,$chk);
...
switch($d){case('parent'):$r=sav::newartparent(); break;
	...
	case('pfunc'):[$plg,$fnc,$prm]=expl('/',$o,3); $r=plugin_func($plg,$fnc,$prm); break;
...
elseif($d=='pclass')$o=3; //elseif($d=='tag')$o=1;// elseif($d=='pfunc')$o=1;
...
elseif($d=='pclass')$o=3; elseif($d=='pfunc')$o=1; elseif($d=='tag')$o=1; 
if(is_array($r))foreach($r as $k=>$v){$c=$k==$vrf?'active':''; $k=addslashes($k);
	if(is_array($v) or is_numeric($v))$v=$k; $v=stripslashes($v);
	if(strpos($d,'|')===false)$t=$k?$k:$d; elseif($k)$t=$k; elseif($vrf)$t=$vrf; else $t='';
...
static function conn2($g1){$d=sql('msg','qdm','v','id='.$g1); 
