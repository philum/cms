...
static function sav($rca,$rcb,$noda,$nodb){
...
$ret=self::sav($rca,$rcb,nod('frnb_'.$p),nod('frnc_'.$p));
return $ret;}

//selected tables
static function build($p,$o){$ret='rr'; //req('msql');
...
reqp('taxonav'); req('spe'); $r=$_SESSION['rqt'];
$r=collect_hierarchie_d('reverse');
//$r=tri_hierarchic($r,$h); p($r);
//taxonomy($r)
//$ida=id_of_ib($k);
//$r=ibofid_r($k,$r);
return self::sav($rca,$rcb,nod('taxo_'.$p),nod('taxo_'.$p));}
