...
class finder{
//utils
static function normalize_s($n){//let the "/"
$n=str_replace([" ","'",'"',"?","§",",",";",":","!","%","&","$","#","_","+","!","\n","\r","\0","[\]","~","(",")","[","]",'{','}',"«","»"],"",($n));
return eradic_acc($n);}
...
return lj($c,'fndr_finder___'.$p.'_'.$o,self::mimes($d,'',16),$d).' ';}
...
if($rb[0]=='disk' && auth(6))$ret=lj('fipop','fndr_finder__3__'.$o,'root').' ';
if($rb[0]=='shared')array_unshift($r,''); $n=count($r); $cur='';
for($i=0;$i<$n;$i++){$cur.=($i&&$n>1?'/':'').$r[$i]; 
	if(!$i)$r[$i]=pictxt('reload',$r[$i]?$r[$i]:'root'); $j=ajx($cur).'_'.$o;
	if($i==$n-1)$c='active';
	if($r[$i])$ret.=lj($c,'fndr_finder___'.$j,$r[$i]).' ';}
return btn('nbp',$ret);}

//virtual_dir
static function virtual_array($r,$o){$ret=[];
...
//distant
static function fdistant($p){reqp('microxml');//server
$h=strto($p,'/'); if(strpos($h,'.')!==false)//unset($r['_menus_']);
return mx_call('http://'.$h.'/?msql=server/shared_files');}
...
if($r)foreach($r as $k=>$v){if($v==$h)$cur='ok';
	if($v)$ret.=divc('fipop',lj('popbt','fndr_finder___'.$v.'_auto',$v));}
if(!$cur)$ret.=blj('','fisrv','fifunc___dist*reg',self::pic('register')).' ';
...
static function rename($d,$id,$res){
$dr='users/'; $res=self::normalize_s(ajxg($res)); $not='';
if(!$res){$j=$id.'firnm_fifunc___rename_'.ajx($d).'_'.$id.'__'.$id.'frnm';
...
if($id!='go')return blj('txtyl',$id.'fidel','fifunc___del_'.ajx($d).'_go',pictxt('alert','really?'));
...
static function vdir($sh,$id,$res){$v=sesr('curdir',$sh);
if(!$res){$j=$id.'fivrd_fifunc___vdir_'.$sh.'_'.$id.'__'.$id.'fvdr';
...
self::distrib_share();
return flapf($dr,'disk/////alone/');}

static function newdir($d,$id,$res){$dr='users/'; $res=ajxg($res);//!is_dir($d)?'../'
if(!$res){$j=$id.'finew_fifunc___newdir_'.$d.'_'.$id.'__'.$id.'fnew';
...
if($id!='go')return blj('popdel',$id.'fidld','fifunc___deldir_'.ajx($d).'_go',pictxt('alert','really delete directory?'));
walk_dir($j,"removef"); rmdir($j);
return self::fparent($d,$id.'fidld','deleted',1);}

static function download($d,$id,$res){$dr='users/'; $drd=self::droot();
if(!$res){$nnm=$_SESSION['qb'].'/downoalds/'.strend($d,'/');
	$j=$id.'fidwn_fifunc___download_'.ajx($d).'_'.$id.'__'.$id.'fdwn';
...
else $w=self::info_dist($f,'fwidth'); return [$w,$h];}
/*static function self::show_img($f,$s){$img=make_thumb_c($f,$s,1); list($w,$h)=self::sizes($f);
return ljb('','SaveBf','photo_'.ajx($f).'_'.($w).'_'.($h),$img);}*/
static function show_img_b($f){list($w,$h)=self::sizes($f); $img=image($f,$w>600?'600px':$w);
return ljb('','SaveBf','photo_'.ajx($f).'_'.($w).'_'.($h),$img);}

static function info_shared($d,$id){
$sh=in_array_r($_SESSION['curdir'],$d,0); $j='fifunc___'; $dj=ajx($d).'_'.$id;
if($sh)$t=nms(74); else $t=nms(75); $c=($sh?'color:#bd0000':''); 
$ret=blj('',$id.'fishr',$j.'share_'.$dj,picto('share',$c)).' ';
if($sh)$ret.=blj('',$id.'fivrd',$j.'vdir_'.$sh.'_'.$id,self::pic('virtual_dir')).' ';
return $ret;}
...
static function reader($d,$dist){req('pop'); $id=randid();
$dr=self::droot(); $f=$dr.$d; $dj=ajx($d).'_'.$id; $xt=xtb($f); $ret='';
//$ret.=blj('',randid().'fidel','fifunc___del_'.ajx($d),self::pic('delete')).br();
if($xt && is_file($f)){
	if(strpos('.jpg.png.gif',$xt)!==false)$ret.=self::show_img_b($f,'').' ';
	elseif(strpos('.mp3.mid.flac',$xt)!==false)$ret.=audio($f);
	else switch($xt){
	case('.txt'):$ret.=lj('','popup_poptxt___'.ajx($f,''),self::pic('pdf',32)); break;
	case('.mp4'):$ret.=lj('','popup_popvideo___'.ajx($f),self::pic('play',32)); break;
	case('.mkv'):$ret.=lj('','popup_popvideo___'.ajx($f),self::pic('play',32)); break;
	case('.ogv'):$ret.=lj('','popup_popvideo___'.ajx($f),self::pic('play',32)); break;
	case('.pdf'):$ret.=lj('','popup_poppdf___'.ajx(host().'/'.$f),self::pic('pdf',32)); break;}}
return self::finfo($d,$id,$f,$dj).br().$ret;}

static function reader_pop($d,$dist){
return self::reader($d,$dist);}

static function dirinfo($d,$o){$id='fed'.normalize($d); $s=strpos($d,'/'); $ret='';
$j='fifunc___'; $jx=ajx($d); //$ret=btn('popbt',pictxt('folder2',$d)).' ';
if($s)$ret.=blj('',$id.'firnm',$j.'rename_'.$jx.'_'.$id,self::pic('rename')).' ';
$ret.=blj('',$id.'finew',$j.'newdir_'.$jx.'_'.$id,self::pic('new')).' ';
if($s)$ret.=blj('',$id.'fidld',$j.'deldir_'.$jx.'_'.$id,self::pic('delete')).' ';
$ret.=self::plnk($d,$o).' ';
$ret.=lj('','ffils_fifunc___sharedir_'.ajx($d),picto('share')).' ';
$ret.=blj('',$id.'upurl','app___finder_upurl_'.ajx($d).'_'.$id,picto('photo')).' ';
$ret.=upload_j('upl'.str_replace('/','',$d),'disk',$d).' ';
return $ret;}

//upurl
static function upurlsav($dr,$o,$u){$u=ajxg($u);
if(!is_image($u))return'no'; $ret=get_file($u); $nm=strrchr($u,'/');
$f='users/'.$dr.'/'.$nm; write_file($f,$ret);
return 'ok';}
static function upurl($p,$id){req('ajxf');
return assistant($id,'SaveJ',$id.'upurl_app___finder_upurlsav_'.ajx($p).'__'.$id,'http://','');}
...
if(!is_numeric($k) or is_array($v)){$rc['r']=1; $rc['f']=$k; $nf=count($v); $nbd=0;
	...
	if(is_file($fb) && $xt)if(strpos('.jpg.png.gif',$xt)!==false && $rb[6]!='pictos' && substr(self::droot(),0,4)!='http')//set as mini
		$rc['img']=make_thumb_c($fb,'48/48',''); //self::show_img($fb,'48/48')
		else $rc['typ']=$xt;
...
foreach($r as $k=>$v){$jx=$v['id'].'_fifunc_';
	list($url,$img,$j,$xt,$dist,$prop,$f,$typ,$id)=vals($v,['url','img','j','xt','dist','prop','f','typ','id']); 
	$ico=$img?$img:mimes($typ,'',18);
	$div=div(atc('fisub').atd($id),'');
	if($v['r'])$lk=toggle('',$jx.'home_'.$j.$o,$f);//'&#9658; '.
	else{$lk=toggle('',$jx.'reader_'.$j.$dist,$f);}
...
foreach($r as $k=>$v){$jx=$v['id'].'_fifunc_'; $div=divd($v['id'],'');
	...
	if($v['r'])$lk=lj('','fndr_home___'.$v['j'].$o,$v['f']);
	else $lk=toggle('',$jx.'reader_'.$v['j'].$v['dist'],$v['f']);
	$ret.=divc('fipop',$ico.' '.$lk.$div);}//.' '.$v['opt']
return $ret;}
...
foreach($r as $k=>$v){
	...
	if($v['r'])$lk='fndr_home__15_'.$v['j'].$o;
	else $lk='popup_fifunc__15_reader_'.$v['j'].$v['dist'].'_1';
...
static function conn($r,$p,$rb){req('spe');
$o=self::mkprm($rb,'alone',5).'_1'; $ret='';//1=popup
foreach($r as $k=>$v){if(isset($v['j'])){$jx='popup_fifunc___';//$v['id'].
	//p($v);
	...
	if($v['r'])$lk=lj('icones',$jx.'home_'.$v['j'].$o,$ico.br().$v['f']);
...
list($r,$rb)=self::slct($p,$rb); if($r)ksort($r);
return self::data($r,$p,$rb);}

static function flapf($p,$o){$r=self::fiflap($p,$o);
...
foreach($r as $k=>$v){if(is_array($v)){$np=$p.'/'.$k; $i++;
	...
	$j.=sj('ffils_fifunc___flapf_'.ajx($np).'_'.$o);
	$lk=ljb('',$j,'',$k);
...
foreach($r as $k=>$v){if(isset($v['r'])){
	list($url,$img,$j,$xt,$dist,$prop,$f)=vals($v,['url','img','j','xt','dist','prop','f']);
	$furl=$jc.$url;
	if($img){list($w,$h)=self::sizes($furl);
	$ico=ljb('','SaveBf','photo_users(slash)'.$j.''.$w.'_'.$h,$img).' ';}
	else $ico=mimes($xt,'',18).' ';
	$op=lkt('txtsmall',$furl,picto('url')).' ';//url
	if($xt=='.mp3')$mp3=1; if($xt=='.jpg')$jpg=1; 
	$lk=lj('','popup_fifunc___reader*pop_'.$j.$dist,etc($f,40)).' ';
	if(!auth(4))$sh=lkc('','plug/download.php?file='.$furl,self::pic('download'));
	elseif($prop==$_SESSION['qb']){
		$sh=self::info_shared($url,randid());//share
		$sh.=blj('',$k.'firnm','fifunc___rename_'.ajx($url).'_'.$k,self::pic('rename')).' ';
		$sh.=blj('',$k.'fidel','fifunc___del_'.ajx($url).'_'.$k,self::pic('delete')).' ';}
...
list($u,$fl,$rg,$ac)=vals($fi,['url','flap','reg','act']);
$ret.=$u.$fl.$rg.$ac;
$ret.=($ret?br():'').($fi['win']??'');
...
static function droot(){return $_SESSION['droot']?$_SESSION['droot']:'users/';}

static function slct($p,$rb){
if($rb[3]=='icon')$_SESSION['droot']=$jc='imgb/icons/'; else $jc='users/';
if(!ses('curdir'))self::shared_files();// or $_GET['admin']=='finder'
...
static function normalize($p,$o){}

static function home($p,$o){
list($p,$o,$ra,$rb)=self::define($p,$o);
list($r,$rb)=self::slct($p,$rb); $fi=[]; $ret='';
...
elseif($rb[4]!='conn' && $rb[5]!='alone')$fi['act']=self::plnk($d,$o);
if($r){$rb[5]=='';//read
	$rr=self::data($r,$p,$rb);
	if($rb[3]=='icons')$fi['win']=icons($rr,$p,$rb);
