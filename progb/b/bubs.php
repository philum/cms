...
$ret[]=[$dir,'link','','/'.$dir,'','','Categorie',sesr('catpic',$dir),'','']; echo $dir;
...
return mod::mkmods(mod::val_to_mod($d));}
...
$r[]=['design','ajax','bubble','bubs,root','msql','',$cat.'/design',''];
$r[]=['gallery','ajax','bubble','bubs,root','msql','',$cat.'/gallery',''];
$r[]=['public','ajax','bubble','bubs,root','msql','',$cat.'/public',''];
if(auth(6)){
$r[]=['admin','ajax','bubble','bubs,root','msql','',$cat.'/admin',''];
$r[]=['connectors','ajax','bubble','bubs,root','msql','',$cat.'/connectors',''];
$r[]=['program','ajax','bubble','bubs,root','msql','',$cat.'/program',''];
$r[]=['system','ajax','bubble','bubs,root','msql','',$cat.'/system',''];
$r[]=['program','ajax','bubble','bubs,root','msql','',$cat.'/helps',''];}
return $r;}
...
foreach($r as $k=>$v)$ret.=popbub('admin',$k,pop::mimes($k).'&nbsp;'.$v,'',1);
return $ret;}
...
if($r)foreach($r as $k=>$v){
	...
	else{if(isset($_SESSION['line'][$v[1]]))$lk=htac('cat').$v[1];
...
if(ses('USE'))$ret=popbub('user','',pop::mimes('login').'&nbsp;'.ses('USE'),'',1);
return $ret;}
...
if(strto($k,'/')==strto($dir,'/')){
	...
	if($va<=$_SESSION['auth']){$t=$rm[$ka]?$rm[$ka]:$ka; $ico=pop::mime($ka);
		if($ka=='css'){//name,j,root,ico,lk
			$mlt='page_deskbkg;popup_admin__3_css;popup_site___desktop_ok';
...
foreach($r as $k=>$v){if($k==$dir)foreach($v as $ka=>$va){$t=$rm[$ka]??$ka;
	if($k!='Microsql')$ret[]=[$t,'ajax','popup','admin___'.ajx($ka),'','',$k,pop::mime($ka)];
	//$ret[]=[$v,'ajax','','admin_'.$ka,'','',$k,pop::mime($v)];//target 'admin'
	//$ret[]=[$t,'link','',htac('admin').$ka,'','',$k,pop::mime($ka)];
...
if($rc[$cond?$cond:'menu']??'' or !$v[5]){$t=$v[0];
	...
	if($dir==$v[6])$is=true; else $is=apps::match_vdir($dr,$nd,$rv);
	...
	if($is && empty($v[8]) && (empty($v[9]) or auth($v[9]??''))){//noj
		...
		elseif($v[1]=='module'){$rb[$t]=mod::mkmodr($v[2]);}//
		else{$j=apps::read($v); $rb[$t]=ljbub($ico.$t,'',sj($j));}}}}
if($rb)$ret=implode('',$rb);
//if($d=='arts')//$ret=apps::ico($rb,'icones');
