...
$ra=install::db(); $sql=$ra['img']; qr($sql);}
...
return conn::place_image($d.'?'.$w,3,'',920,'');}

static function png2jpg($a,$id){
$b=str_replace('.png','.jpg',$a);
$in='img/'.$a; $out='img/'.$b;
[$w,$h,$ty]=getimagesize($in);
...
return conn::place_image($b,3,'',920,'');}
...
if((!file_exists($thumb) && $mg && $pre) or ses('rebuild_img')){
	[$w,$h]=explode('/',prmb(27)); $mode=$_SESSION['rstr'][16]?0:1;//0=outsize
