...
if(substr($v[1],0,1)!='_' && $v[7]){$i++;
	...
	if($prw!='full')$msg=substr($msg,0,kmax_nb(400,$msg));
	$msg=conn::read($msg,$prw,$k,'nl'); //$msg=codeline::parse($msg,'','sconn');
...
foreach($r as $k=>$v){
	...
	else{if($n=strpos($k,' '))$ret.=bal(substr($k,0,$n),substr($k,$n+1),$v)."\n";
		else $ret.=balb($k,$v)."\n";}}
return $ret;}
