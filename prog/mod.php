...
function mod_load($load,$ret,$t,$d,$o,$obj,$prw,$tp,$id){//todo:opt_selector
...
$in=read_msg($_SESSION['frm'],1,'');
if($in)return divc($o,$in);}
...
function usertags($v){$nbdays=getorpost('dig',ses('nbj')); $daybb=calc_date($nbdays);
...
function trkarts($t){
$nbdays=getorpost('dig',$_SESSION['nbj']); $daybb=calc_date($nbdays);
...
return array($ret,lka(htac('source').strdeb($src,'.'),$t?$t:$src));}
...
$ret=autoclic('search" id="'.$id.'" '.$js,$t,$s,'100','');
...
if(trim(strip_tags($ret))){
	if($t)$reb=divc("txtcadr",stripslashes($t)).br();
	$ret=$reb.$ret.($nbr?br():'');}
$_SESSION['cur_div']='content';
