...
[_BACK§txtbox:css] _AVATAR [_AUTHOR _DATE§txtsmall2:css] [_NBARTS§txtnoir:css] [_LENGHT _PRIORITY _SOURCE _USERTAGS _TAG _BTIM _RSS _TRACKS _OPT _PID _LANG§txtsmall2:css]§div|meta_ID:balise]
...
[_BACK§txtbox:css] [_AUTHOR _DATE§txtsmall2:css] [_NBARTS§txtnoir:css] [_LENGHT _PRIORITY _SOURCE _USERTAGS _TAG _BTIM _RSS _TRACKS _OPT _PID _LANG§txtsmall2:css]§header|meta_ID:balise]
_FLOAT[_MSG[:clear]§article|art_ID|justy:balise]';}//_TRKBK
...
function template_pubart(){return '[[_IMG1§44/44:thumb]§[imgr:class]:div][_CAT§txtx:css] [_DATE§txtsmall2:css][[_JURL§_SUJ:jurl]§div||:balise]_VIDEO[:clear]';}
...
if(is_numeric($id))$ret.=tracks_read($id,$page,$nb);
return $ret;}
...
$ret=template($panout,'tracks');
return divd('trk'.$id,$ret);}

function urledt_id($id){$b=rstr(18)?'public':$_SESSION['qb'];
$u=is_numeric($id)?rqt($id,9):$d; list($k)=verif_defcon($u);
...
$ret.=$alrt;
$ret.=divd('txarea',txarea1($msg,$cont));
