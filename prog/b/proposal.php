...
$inp2=!$usr||auth(6)?inputj('inp2',$usr?$usr:'user',$j,atz(8)):hidden('','inp2',$usr).btn('txtx',$usr);
$ret.=inputj('inp1','msg',$j,atz(48),1).$inp2.lj('popbt',$j,picto('save'));
...
if($r)foreach($r as $k=>$v){$id=self::valid($p.$v[1]); $edt=favs_edt($id,'agree',$rf[$id]??''); $del='';
...
static function call($p,$o){req('art'); $ord='';
...
static function add($p,$o,$res=''){
list($p,$o)=ajxp($res,$p,$o); $k='';
...
$ret=msqbt('',$nod);
$ret.=input('inp',$p);
$ret.=lj('popbt',$rid.'_proposal,add__3__'.$rid.'_inp',pictxt('add','new job'));
