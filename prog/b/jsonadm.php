...
static function create($u,$rid,$res=''){
if(substr($u,0,1)=='/')$u=substr($u,1); //echo $u;
$r=explode('/',$u); $root='json'; $vb=''; //p($r);
list($dr,$nod)=self::drnod($u); $nod.='/'.ajxg($res);
json::read($dr,$nod,$rid);
return self::nav($u,$rid);}

static function add($u,$rid){
$ret=inputj('add'.$rid,'',$rid.'_jsonadm,create___'.ajx($u).'____add'.$rid);
...
list($dr,$nod)=split_right('/',$f);
$ret=btn('popw',$dr.','.$nod);
return $ret.self::read($dr,$nod,$rid);}

static function call($p,$rid,$res=''){
$f=ajxg($res);
$ret=self::build($f,$rid);
return $ret;}

static function nav($u,$rid){$ret='';
list($dr,$nod,$u)=self::drnod($u); $root='json';
$r=scandir_b($root);
