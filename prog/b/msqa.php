...
$ret.=lj('popbt','admsql_msqa,find_msqsr__'.ajx($dr).'_'.ajx($nd),nms(24));
return $ret;}
...
static function msqcall($g1,$g2,$g3,$g4){$ret=''; $r=msql::read($g1,$g2,$g3); $v=$r[$g4]??($r[0]??'');
...
return divc('small',$b.conn::parser(helps($g1)));}

#draw 
static function tables($base){$r=explore($base,'files',1);
...
if(strpos($d,'<')!==false or strpos($d,'>')!==false)$d=htmlentities_a($d);//$d=htmlentities_b($d);
...
$ret=pop::btpages($npg,$page,$n,'msqdiv_msqa,home___'.ajx($murl).'_');//!
...
[$dr,$nd,$n]=msqa::murlvars($g1); $d=$prm[0]; $d=deln($d); $d=delbr($d,"\n");//$d=delr($d);
msql::modif($dr,$nd,trim(strip_tags($d)),'shot',$g3,$g2);
return msqa::mdfcolbt(nl2br(msqa::cutat($d)),$g2,$g3,$g1,$g4);}

/**/static function msqlopen($g1,$g2,$g3,$g4){
if($g4)$g4=':'.$g4; [$w,$h]=explode('-',get('sz')); $s=$w?$w:720; ses::$r['popw']=$w;
$url='/msql/'.($g1=='lang'?$g1.'/'.prmb(25):$g1).'/'.$g2.($g3?'_'.$g3.$g4:''); 
return iframe($url,$s-20,$h-40);}
...
if(is_array($r)){$i=0; $kys[]='k'.$rid;//keys to shift from array
	foreach($r as $k=>$v){$kb=self::normaliz($idn.$k); $kys[]=$kb; $i++;
...
$bt=lj('popbt','popup_msqa,editmsql___lang/'.$lg.'/helps*txts_'.ajx($va),$lg);}
if(auth(4)){
	...
	$bt.=lj('popbt',$tg.'_msqa,msqlsav_'.$keys.'_x_'.$nodb.'_@'.$kyb,nms(44)).' ';//new
	$bt.=$pn.' ';//prevnext
	...
	$bt.=lj('popdel',$tg.'_msqa,msqldel___'.$nodb.'_'.$kyb,pictit('del',nms(76))).' ';}//del
...
if($md=='creatable'||$md=='del_file'||$md=='del_backup'||$md=='rename_table'||$md=='duplicate_table')return $r;//reload//
...
if($md=='export_csv'){[$dr,$nod]=self::node_decompil($p); $r=msql::read($dr,$nod); return csvfile($nod,$r,$nod,1);}
...
if($md=='import_csv')$d=self::edtcsv($r);
...
static function edtcsv($r){$rc=[]; //return array2csv($r);//import_csv
...
if(strpos($d,'msql/')!==false){$r=explode('/',$d); $n=count($r)-1; $nod=$r[$n]; $dr=$r[$n-1];
	$u=http(prms('origin')).'/call/msqj/'.$dr.'|'.$nod; $r=self::import_json($u);
...
static function repair_table($r,$dr,$nod){
foreach($r as $k=>$v){if(!is_array($v))$v=[$v]; if($k && $v[0])$rb[$k]=$v;}
...
if($a=='design')return styl::append_design($defs,$r);
...
[$node,$line]=split_one(':',$node,1);
[$b,$d]=split_one('/',$base,0); 
[$p,$t,$v,$l]=expl('_',$node,4); $l=$l?$l:$line;
...
static function murl($b,$d,$p,$t,$v){
if($_SESSION['htacc'])return ($b?$b.'/':'').($d?$d.'/':'').self::mnod($p,$t,$v);
else return '/?msql='.$b.'&dir='.$d.'&msql='.self::mnod($p,$t,$v);}
...
#boot
static function boot($msql){$auth=$_SESSION['auth']; $ath=6; $root='msql/';//self::sesm('root')
$_SESSION['murl']=self::murlread($msql);
[$b,$dir,$hub,$table,$version,$def]=$_SESSION['murl'];
...
$ra[6]=$files;
	if($files && $auth<=$ath){foreach($files as $k=>$v){
		...
		elseif($k==$_SESSION['USE'])$filb[$k]=['public'];
...
#boot
if($msql && $msql!='='){
	if($_SESSION['htacc'])$url=self::sesm('url','/msql/'); else $url=self::sesm('url','/?msql=');
	$ra=self::boot($msql); $_SESSION['msql_boot']=$ra;
...
if(!$def && auth(6)){
	...
	if($auth>=$ath && $table && $hub && $is_file){
		...
		if(auth(6))//($base=='system' or $hub=='public') && 
			$ret['fls'].=self::opbt('renove',$jurl,['renove','import from '.prms('origin')]);
...
if($table && $is_file){
	...
	if($authorized)//add
		$ret['nfo'].=lj('txtblc','popup_msqa,editmsql___'.$jurl.'_add',pictit('add',$lh[28][0])).' ';
	$ret['nfo'].=lj('txtx','admsql,editable___'.$jurl,picto('refresh')).' ';
	$wurl=$_SERVER['HTTP_HOST'].'/msql/'.$murl;
	$ret['nfo'].=lj('popbt','popup_usg,txt___'.ajx($wurl).'_console',pictit('link','web url'));
