...
static function admactbt($p,$t,$x='',$o='',$tt='',$c='txtx'){
...
$ret.=console::admactbt('newfrom_mods','','self',$nw,nms(99).':'.$nw);//new
$prmb=sql('config','qdu','v','name="'.ses('qb').'"');
if($prmb)$prmb1=strprm($prmb,1,'#'); else $prmb1='';
if($prmb1!=prmb(1))$ret.=console::admactbt('adopt_mods',nms(66),'self');//apply
...
$ret.=console::admactbt('backup_mods',$rt['backup'],'','',nms(94));
if(is_file($f))$ret.=console::admactbt('restore_mods',$rt['restore'],'self','',nms(95));
$ret.=console::admactbt('refresh_mods',$rt['refresh'],'self','',nms(97));
if($p1=ses('prmb1'))$ret.=console::admactbt('make_copy',$rt['copy'],'','',nms(132));
$ret.=console::admactbt('default_mods',$rt['default'],'','',nms(96));
$ret.=console::admactbt('mk_default',$rt['mkdef'],'','',nms(113));
$ret.=hlpbt('console').' ';
...
$ret=console::admactbt('bckp_cnfg','backup');
if(is_file($f))$ret.=console::admactbt('restore_cnfg','restore','self');
$ret.=console::admactbt('reset_cnfg','reset','self');
$ret.=console::admactbt('cnfgxt','txt');
return $ret;}
...
if($ret)$ret=lj($cnd[0]?'':'active','mdls'.$vl.'_modsav___'.$vl.'__all','-').$ret;
return divc('nbp',$ret);}}
...
$ret.=console::admactbt('set_cond',$k,'admcnt',$k,'',$c);}}
if($ret){$all=console::admactbt('set_cond',nms(100),'admin','all');
...
$mb=pop::mimes($m,'file-config'); return $mb.' '.$m;}
...
$ret.=lj($css,'popup_admx,configmod___'.$k,self::build_mod_subname($v));}
return $ret;}
...
$ret.=lj('','popup_admx,addmod___'.$vl,picto('plus')).' ';//bar_add_mod
...
foreach($r as $k=>$v)if($v && $v!='clear'){if($v=='system')$hlp=hlpbt('blocsystem');
	...
	$ret.=lj('txtx','mdls'.$v.'_modsav___'.$v.'__'.$_SESSION['cond'][0],$v).$hlp;
	$ret.=divc('menu',divd('mdls'.$v,self::block($v)));}
$ret.=lj('txtx','mdlsdev_modsav___dev',nms(148)).hlpbt('bloctest');
