...
static function conns($p,$va,$pid='',$rid=''){$ret=''; switch($p){
case('url'):$ret=self::url($va,$rid); $t='URL or ID article'; break;
case('art'):$ret=self::art($va,$rid); $t='ID article'; break;
case('img'):$ret=self::upload($va); $t=nms(78); break;
...
case('module'):$ret=self::editcomm($p); break;
case('ajax'):$ret=self::editcomm($p); break;
case('articles'):$ret=self::editcomm($p); break;
...
case('preview'):$t=nms(65); $va=embed_links($va);
...
//command
static function editcomm($d){
if($d=='module')return admx::mod_edit_pop(1);
if($d=='ajax')return admx::mod_edit_pop(0);
if($d=='articles')return admx::artmod_edit($d);}
...
static function vacuum(){$id=$_SESSION['read'];
...
$ret.=toggle('txtx',$id.'up_sav,placeim_'.$id,'portfolio').' ';
$ret.=upload_j($id,'art');
return divs('width:320px;',$ret);}
...
if($va=='cleanbr')$rt=clean_br($d);
elseif($va=='cleanmail')$rt=cleanmail($d);
elseif($va=='cleanpunct')$rt=clean_punct($d,2);
elseif($va=='cleanpdf')$rt=self::clean_pdf($d);
elseif($va=='striplink')$rt=codeline::parse($d,'striplink','correct');
elseif($va=='converthtml'){$rt=conv::call(nl2br($d)); $rt=post_treat_repair($rt);}
...
elseif($va=='randim'){geta('randim',1); $read=$_SESSION['read'];
	if(is_numeric($read))update('qdm','msg',$d,'id',$read);
	$ret=conn::read($d,3,$read);
	$rt=sql('msg','qdm','v','id='.$read); getz('randim');}
...
elseif($va=='inclusive')$rt=clean_inclusive($d);
...
for($i=1;$i<200;$i++){
$types=['[['.$i.':nh]]','(['.$i.':nh])','['.$i.':nh]','[['.$i.':nb]]','(['.$i.':nb])','['.$i.':nb]','[['.$i.']:b]','[['.$i.']:i]','[['.$i.']]','['.$i.'.]','['.$i.']','['.$i.':e]',"\n".$i.'.',"\n".$i];//'|'.$i.'|'
...
//dsnav
static function conn_props_b($n){$ret='';
...
//ascii
static function ascii4($p,$id){$ret=lj('txtx','nvascii4_mc,navs___ascii4__'.$id,picto('back'));
...
$ret=lj('txtx','nvascii4_mc,navs___ascii4_'.$id,picto('back')).' ';
$ret.=lj('txtx','nvascii4_mc,unicode____'.$id,'nocat').' ';
$rc=msql::cat($r,3); foreach($rc as $k=>$v)$ret.=lj('txtblc','nvascii4_mc,unicode___'.$k.'_'.$id,$k).' ';
...
if($op=='ascii4'){$r=msql::read_b('system','edition_ascii_11','',1); $r=msql::cat($r,1);
	$ret.=lj('','nv'.$op.'_mc,unicode___'.$id,picto('globe')).' ';
	foreach($r as $k=>$v)$ret.=lj('','asc4_mc,ascii4___'.$k.'_'.$id,$k).' ';
	$ret.=lj('txtx','popup_ascii,home',picto('icons'));
...
elseif($op=='uc'){$r=msql_read('',ses('qb').'_connectors_1','',1);
...
elseif($op=='backup'){//$read=$id?$id:ses('read');
	...
	if($read){$r=msql_read('',$nod,'',1); $k=0; $rb=[];
