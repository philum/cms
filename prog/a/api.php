...
if($r)foreach($r as $k=>$v){$re=[];
	foreach($v as $ka=>$va){
		...
		if($ka=='image' && $va){$va=host().'/img/'.pop::art_img($va);}//$ka='lead_image_url';
...
#build
static function build($r,$ra){$n=count($r); $rm=[]; $rd=[];
$pr=art::slct_media($ra['preview']??''); $tp=$ra['template']??''; $nl=$ra['nl']??''; $cmd=$ra['cmd']??'';
if($rch=$ra['search']??''){$pr='rch'; geta('search',$rch); ses::$r['look']=$rch;}
if(isset($ra['random'])){$n=array_rand($r); $rb[$n]=$r[$n]; $r=$rb;}
if($cmd=='panel')foreach($r as $k=>$v)$re[]=mod::pane_art($k,'',$tp);//cmd panel/cover
elseif($cmd=='tracks')foreach($r as $k=>$v)//cmd tracks
	$re[]=art::playb($k,1).art::output_trk(ma::read_idy($k,'asc'));
	...
	foreach($rc as $k=>$v){$v['img1']=pop::art_img($v['img']);
		$re[]=art::build($v['id'],$v,$rm[$k]??'',$prw,$tp,$nl);}}
if(!empty($ra['cols']))return pop::columns($re,$ra['cols']);
return implode('',$re);}
...
$r['all']=nms(100);
$j=$ra['rid'].'_api2_hid'.$ra['rid'].'_exs__';
...
for($i=0;$i<$nb;$i++){$k=$rb[$i]; $v=$r[$k];
	...
	$ret.=ljp($p,$j.ajx($k),$k>360&&$k!='all'?$dt:$v).' ';}}
...
$nbyp=$ra['nbyp']??20; $pg=$ra['page']?$ra['page']:1; $ret=''; $prw='';
if($ra['nbarts']>$nbyp)$nbp=ceil($ra['nbarts']/$nbyp);
if(isset($nbp))$rp=self::pages_nb($nbp,$pg);
$j=$ra['rid'].'_api2_hid'.$ra['rid'].'_exs_';
if(rstr(110)){$prw=art::slct_media($ra['preview']??''); $prwa=art::slct_media($ra['prw']??'');
	if($prwa!=1)$ret=ljp(atc(active($prw,$prwa)),$j.$pg.'__'.$prwa,picto('preview',16)).'';
	$ret.=ljp(atc(active($prw,1)),$j.$pg.'__1',picto('filelist',16)).' ';}
if(rstr(53)){$lg=$ra['lg']??''; $lng=ses('lng');
	if($lg==$lng){$c=atc('active'); $prb=$prw.'-';} else{$c=''; $prb=$prw.'-'.$lng;}
	$ret.=ljp($c,$j.$pg.'__'.$prb,flag($lng)).' ';}
...
if($t && isset($ra[$t])){$ti=!empty($ra['t'])?$ra['t']:$ra[$t];//used for num tags
...
if(strpos($prw,'-')){[$prw,$lg]=explode('-',$prw); $ra['lg']=$lg;}
...
elseif(isset(ses::$popadm['timetravel'])){$ra['maxtime']=ses('daya');}
else return;
...
$com=implode_k($ra,',',':');
$bt=hlpbt('api').' '.lj('grey','popup_favs,home___com_'.ajx($com),picto('save')).' '.lj('grey','popup_apicom,home___'.ajx($com),picto('view')).' '.
