...
class bubs{
//search
static function search_btn(){
req('mod'); return search_btn();}

//addart_fast
static function addart_btn(){req('ajxf');
return bub_adm_addart();}

//ucom
static function ucom_btn(){$j='sjtime(\'ucom\',\'socket_ucom_ucom_url\');';
$js='onClick="'.$j.'" onContextMenu="'.$j;
//$js='onClick=sj(this); onContextMenu=sj(this); onkeypress=ckeckj(self,this);'.atb('data-j',$j);
//$js=atb('onClick',$j).atb('onContextMenu',$j).atb('onkeyup','checkj(self,this)').atb('data-j',$j);
...
$rb=msql_read('lang','helps_langs','');
foreach($r as $v){$ico=$v==$_SESSION['lang']?'valid':'link';
...
if(auth(6)){
	if(rstr(99))$ret[]=['twitletter','ajax','popup','plug__3_tweetfeed_tweetfeed*batch','','','dev','tw2'];
	if(prms('srvmirror'))
	$ret[]=['transport','ajax','popup','plup___transport','','','dev','exchange'];
	$ret[]=['push','ajax','popup','plup__3xx_dev2prod','','','dev','upload'];
	$ret[]=['publish','ajax','popup','plug___pubdate','','','dev','export'];
	if(!prms('aupdate'))
	$ret[]=['update','ajax','popup','software,home','','','dev','update'];
	if(ses('rebuild_img')){$bt='-off'; $n=0;} else{$bt=''; $n=1;}
	$ret[]=['mini'.$bt,'ajax','socket','sesmake___'.$n.'_rebuild*img','','','dev','img'];
	$ret[]=['refresh','ajax','socket','reset__self','','','dev','refresh'];}
$ret[]=['cache','ajax','popup','rebuild__3','','','dev','reload'];
if(auth(2))$ret[]=['push','ajax','popup','plup__3xx_dev2prod','','','dev','down'];
return $ret;}

//time
static function timetravel(){$r=timetravel(); $travel=date('Y',ses('daya'));
$ret[]=[date('Y'),'link','','/reload','','','','logout'];//nms(83)
foreach($r as $k=>$v)$ret[]=[$k,'link','',target_date($v),'','','',$travel==$k?'clock':'hour'];
return $ret;}
...
$rb=ses('catpic',[]); $ret='';
if($ra)foreach($ra as $k=>$v){$ic=isset($rb[$k])?$rb[$k]:'folder';
	$ret.=popbub('arts',ajx($k),picto($ic).'&nbsp;'.$k,'c',1);}
return $ret;}

//seek
//tag find arts
static function seek_art($d){req('mod'); list($cat,$tag)=explode('-',$d);
$r=tag_arts($tag,$cat,7); unset($r[ses('read')]);
if($r)foreach($r as $k=>$v)
	$ret[]=[suj_of_id($k),'art','',$k,$d,'',$d,'article'];
return $ret;}
//class find tags
static function seek_tag($d){req('mod'); $id=ses('read');
$r=classtag_arts($d);
...
return $ret;}
static function seek(){$r=explode(' ','tag '.prmb(18)); req('art');
	foreach($r as $k=>$v)$ret=seek_merge($v,$ret);
	//if($v)$ret[]=[$v,'bub','seektag',ajx($v),'','','','folder2'];
return $ret;}

//mods
static function adm_mods($d){req('mod'); //build_modules($va,$cr);
...
//bub selector
static function slct($j){req('ajxf');
...
$r[]=[$qb,'ajax','bubble','popbub','msql','',$cat.'/'.$qb,''];
$r[]=['design','ajax','bubble','popbub','msql','',$cat.'/design',''];
$r[]=['gallery','ajax','bubble','popbub','msql','',$cat.'/gallery',''];
$r[]=['public','ajax','bubble','popbub','msql','',$cat.'/public',''];
if(auth(6)){
$r[]=['admin','ajax','bubble','popbub','msql','',$cat.'/admin',''];
$r[]=['connectors','ajax','bubble','popbub','msql','',$cat.'/connectors',''];
$r[]=['program','ajax','bubble','popbub','msql','',$cat.'/program',''];
$r[]=['system','ajax','bubble','popbub','msql','',$cat.'/system',''];
$r[]=['program','ajax','bubble','popbub','msql','',$cat.'/helps',''];}
return $r;}
...
static function adm_admin_fast(){//$arw=rightarrow();$arw.
//req('admin'); return admin();
$r=msql_read('lang','admin_menus','',1); $ret='';
foreach($r as $k=>$v)$ret.=popbub('admin',$k,mimes($k).'&nbsp;'.$v,'',1);
return $ret;}

//login
static function exec($d){
if($d=='login'){req('pop'); login::call('','',''); return div(atd('nob'),login::form('','1',''));}
if($d=='cache'){req('boot,spe,art'); $_SESSION['rqt']=[]; return li(cache_arts(1));}}
...
//overcat
static function surcat_list(){//surcat_list in spe
$r=sql('msg','qdd','rv','ib="'.ses('qbd').'" and val="surcat"'); $rb=[];
...
//bubmenu
static function menu($d,$n){//root,action,type,button,icon,auth
//if($_SESSION['mnbb'][$b])return $_SESSION['mnbb'][$b];
$r=msql::read('users',nod('menubub_'.$n),'',1); $ret=[];
if(!$_SESSION['line']){req('boot'); define_cats_rqt();}
if($r)foreach($r as $k=>$v){
	list($v0,$v1,$v2,$v3,$v4,$v5)=arr($v,6);
	$bt=$v[3]?$v[3]:$v[1];
	//if($v[2]=='bub')p($v);
	if($v[2]=='plug')$ret[]=[$v[3],$v[2],$v[1],'','','',$v[0],$v[4],'',$v[5]];
	elseif($v[2]=='app')$ret[]=[$v[3],$v[2],$v[1],'','','',$v[0],$v[4],'',$v[5]];
	elseif($v[2]=='appin')$ret[]=[$v[3],$v[2],$v[1],'','','',$v[0],$v[4],'',$v[5]];
	elseif($v[2]=='module')$ret[]=[$v[3],'module',$v[1],'','','',$v[0],$v[4],'',$v[5]];
	elseif($v[2]=='ajax')$ret[]=[$v[3],'ajax',$v[1],'','','',$v[0],$v[4],'',$v[5]];
	//elseif($v[2]=='bub')$ret[]=[$v[3],'bubble',$v[1],$v[0],$v[0],$v[4],'',$v[5]];
...
return $ret;}
static function adm_user(){$rb=msql_read('system','default_apps_user','',1);
...
if(is_array($rb))foreach($rb as $kb=>$vb){list($m,$p,$t,$c,$e,$g,$ch,$h)=$vb;//module
	if($m=='apps')$ret[]=['Apps','ajax','popup','call___adminx_submod*pop','','',$dir.'/'.$v,'apps'];
	else $ret[]=[$m,'ajax','popup','module__3_'.$kb,'','',$dir.'/'.$v,'divide'];}}
return $ret;}
...
foreach($r as $k=>$v){
$ret[]=[$k,'ajax','popup','rstr__3_','','',$dir,'divide'];
...
$r=self::adminauthes2(); $rm=msql_read('lang','admin_authes','',1);
...
if(strto($k,'/')==strto($dir,'/')){
	...
	if($va<=$_SESSION['auth']){$t=$rm[$ka]?$rm[$ka]:$ka; $ico=mime($ka);
		...
		elseif($ka=='tickets')$ret[]=[$t,'plug','chatxml','tickets','','',$k,'chat'];
...
//$ret[]=['system','ajax','popup','msql___system','','','Microsql','url'];
foreach($r as $k=>$v){if($k==$dir)foreach($v as $ka=>$va){$t=$rm[$ka]??$ka;
	if($k!='Microsql')$ret[]=[$t,'ajax','popup','admin___'.ajx($ka),'','',$k,mime($ka)];
	//$ret[]=[$v,'ajax','','admin_'.$ka,'','',$k,mime($v)];//target 'admin'
	//$ret[]=[$t,'link','',htac('admin').$ka,'','',$k,mime($ka)];
...
if($rc[$cond?$cond:'menu']??'' or !$v[5]){$t=$v[0];
	...
	if($is && empty($v[8]) && (empty($v[9]) or auth($v[9]??''))){//noj
		...
		elseif($v[1]=='module'){$rb[$t]=build_mod_r($v[2]);}//req('mod'); 
...
$rb=self::r_apps_cond('home'); if(!rstr(56))$r=unset_in($r,'hubs',0);
//if(!rstr(48))$r=unset_in($r,'boot',6);
return array_merge_b($rb,$r);}

static function root($d,$dir=''){$r=[];
switch($dir){//pre-rendered, intercepte navigation
case('batch'):req('tri,pop,ajxf'); return batch('','c');break;
case('fastmenu'):return self::adm_admin_fast();break;//admin menus
case('search'):return self::search_btn('',18,'srchb');break;
...
case('table'):$r=msql_read('',ses('qb').'_'.$dir,'',1); break;
...
case('menubub'):$r=self::menu($dir,1); break;
case('menubub1'):$r=self::menu($dir,1); break;
case('menubub2'):$r=self::menu($dir,2); break;
case('menubub3'):$r=self::menu($dir,3); break;
case('menubub2'):$r=self::menu($dir,4); break;
