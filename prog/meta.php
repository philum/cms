...
if($suj=quotes(getjx('suj'))){$sql.='suj="'.$suj.'", '; cache_value($id,2,$suj);}
...
$ret.=lj('poph','popup_placeim___'.$id,'images').balise("input",array('','text','','img'.$id,$img,$csb,'36',255,$nk),'').lj('poph','img'.$id.'_recenseim__4_'.$id,picto('up')).br();//img
$ret.=lj('poph','','source').balise("input",array('','text','','src'.$id,$src,$csb,'36',255,$nk),'').br();//src
$ret.=cmption('cat',$id,$frm,0);
$ret.=cmption('tag',$id,$thm,1);
...
switch($d){
	...
	case('tag'):$r=rech_meta('tag',90); break;
...
function cmption($d,$id,$v,$p){static $n; $n++;
...
$etc=lj('','popup_cmption___'.$d.'_'.$n.$id.'_'.$p,picto('folder2'));//...
...
if($chsup)foreach($chsup as $k=>$v)$ret.=cmption($v,$id,$r[$v],1);
return $ret;}
...
if($n==2){$tagtype="tags"; //$words=$_SESSION['interm']; 
	$words=sql('thm','qda','k','nod="'.$_SESSION['qb'].'" AND day>"'.calc_date(90).'"');
