...
$r['priority']='1|2|3|4';//multi
//$r['owner']='user';//published by user
return self::form($r,$p,$id);}

static function build($p,$o,$prm=[]){
$p=$prm[0]??$p;
return api::call($p,$o);}

static function cat(){return ses('line');}

static function form($r,$p,$id){
...
$ret.=lj('',$rid.'_apicom,build_inp',picto('ok')).' ';
$ret.=hlpbt('api').' ';
//$ret.=ljb('','apijumpall',implode_k($r,',',':'),picto('after')).br();
$ret.=divd('loadself','');
$r=self::ex();
$rt=self::form($r,$p,'inp');
...
else $bt=toggle('',$rid.'2_apicom,menu_'.ajx($p).'_'.$rid,picto('menu'));
if($p)$ret=self::build($p,''); else $ret='';
