...
$ra=[":j']",':p]',':vid§o]',':efecto','[///img/',':centro]',':non]',':not]'];
...
#translate
static function read($p,$o='plain',$res='',$to=''){return $res;}
...
[$lg,$from]=opt($setlg,'-'); if($lg=='all')$lg=$from;
...
//$ret=lj('','ynd'.$ref.'_yandex,call__1_'.$ref.'_'.$setlg,picto('kleft')).' ';
$ret=lj('txtx','ynd'.$ref.'_yandex,resav_rdt'.$ref.'_1_'.$ref.'_'.$setlg,pictxt('save',$lg)).' ';
...
//[$ex,$exlg]=sql('txt,lg','ynd','r','ref="'.$ref.'" and lang!="'.$to.'"');
//if(!$ret){}
if($ind=='art'){$ret=sql('msg','qdm','v','id='.$id); $lg=sql('lg','qda','v','id='.$id);}
elseif($ind=='trk')[$ret,$lg]=sql('msg,lg','qdi','w','id='.$id);
elseif($ind=='suj')[$ret,$lg]=sql('suj,lg','qda','w','id='.$id);
...
[$to,$from,$nobt]=opt($setlg,'-',3); $bt=''; //if($to=='all')$to=$from;
if(!$to)$to=ses('lng');//$to=setlng($to);
...
$ret=lj('popbt',$ref.'_yandex,resavxt_rdt'.$ref.'__'.$ref.'_'.$setlg,pictxt('save',$to)).' ';
...
if(auth(6)){$bt.=lj('',$j.'calltw_1__'.$id.'_'.$setlg.'-1',picto('refresh')).' ';
	$bt.=lj('','popup_yandex,redit__1_'.$ref.'_'.$setlg,picto('edit')).' ';}
if($o==2)$bt='';
return $bt.$ret;}

#umrec
static function callum($ref,$setlg,$edt='',$ret=''){
[$to,$from]=opt($setlg,'-'); if($to=='all')$to=$from; $bt='';
if(!$to)$to=ses('lng');//$to=setlng($to);
...
$rid=randid('yd');
$ret=textarea('txt',$p);
$ret.=lj('popbt',$rid.'_yandex,read_txt_1',nms(153).' '.self::$motor);//translate
$ret.=lj('popbt',$rid.'_yandex,detect_txt_1','detection');
$ret.=lj('popbt',$rid.'_yandex,getlangs','langs');
return $ret.divd($rid,'');}
