...
if(!empty($r[0]) && $r[0]==$day)$d=$r[1]??''; //eco($d);
$f='http://logic.ovh/api/meteo/uid:13,insee:'.($p?$p:$insee); //echo $f; //$d='';
if(!$d){$d=get_file($f); $r=json_decode($d,true); if(!is_array($r)){$d=''; $r=[];}
	json::add('','meteo',[$insee,$day,$d]);
	if($d && $d!='null')msql::modif('',nod('meteo_1'),[$day,$d],'row','',$insee);}
